<?php
/*
Functions file for functions associated with a specific user.
*/
...
class loggedInUser {
	...
	public function updateEmail($email)
	{
		...
		$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
			...
			WHERE
			id = ?");
		$stmt->bind_param("i", $email, $this->user_id);
		$stmt->execute();
		$stmt->close();	
	}
	...
	//Get an array of sensors owned by user
	public function getUserSensors()
	{
		global $mysqli,$db_table_prefix;
		...
		while ($stmt->fetch())
		{
        		$sensors[] = array('id' => $s_id, 'coordinates' => $coord, 'latitude' => $lat, 'longitude' => $long);
    		}
		$stmt->close();
		return ($sensors);
	}
	//Add a sensor node to user's account
	public function addSensor($latitude, $longitude)
	{
		if(is_numeric($latitude) && is_numeric($longitude))
		{
			$location = $latitude.",".$longitude;
			global $mysqli,$db_table_prefix; 
			$stmt = $mysqli->prepare("INSERT INTO ".$db_table_prefix."sensors (
				user_id,
				coordinates,
				latitude,
				longitude
			...
			$stmt->bind_param("isss", $this->user_id, $location, $latitude, $longitude);
			$result = $stmt->execute();
			$stmt->close();	
			return $result;
		}
